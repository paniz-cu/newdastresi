    ...
    try {
        ...
        productHTML = products
          ...
          <div class="bg-white p-18 rounded-2xl hover:shadow-2xl cursor-pointer w-full lg:w-[150%] ">
            ...
            <p class="text-red-500 font-bold">${Number(product.discount).toLocaleString()} تومان تخفیف</p>
            <div class="flex justify-between items-center">
              <span class="text-blue-600 font-bold text-lg">${Number(product.price).toLocaleString()} تومان</span>
              <span class="text-gray-400 line-through">${Number(product.oldPrice).toLocaleString()} تومان</span>
            </div>
          </div>
        </div>`;
                return "";
